...
Cho $A$ là một ma trận vuông kích thước $n\times n$. Ma trận con (minor) của $A$ tương ứng với phần tử $a_{i,j}$, ký hiệu là $M_{i,j}$ được suy ra bằng cách bỏ hàng $i$ và cột $j$ trong ma trận $A$.
...
Biểu thức $(-1)^{i+j}a_{i,j}\|M_{i,j}\|$ còn được gọi là _phụ đại số_ (cofactor) của phần tử $a_{i,j}$, ký hiệu $C_{i,j}$. Như vậy, công thức tính định thức ma trận $A$ có thể phát biểu gọn: "Định thức ma trận $A$ bằng tổng các phụ đại số trên một hàng hoặc một cột bất kỳ."
